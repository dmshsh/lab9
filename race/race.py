...
# задаём классы с характеризацией движения каждого
...
class Coin(pygame.sprite.Sprite):
    def __init__(self):
        ...
        self.rect = self.image.get_rect()
        self.rect.center = (random.randint(40, SCREEN_WIDTH - 40), 0)
        self.weight = random.randint(1, 5)  #рандомный вес монет
...
while True:
    ...
    for coin in coins:
        if pygame.sprite.spritecollideany(P1, coins):
            coin_sound.stop()
            coin_sound.play()
            COINS += coin.weight  #добавление веса
            for coin in coins:
                coins.remove(coin)
